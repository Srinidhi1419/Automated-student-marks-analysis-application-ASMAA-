 ...
 * @author Prudvi
 */
@WebServlet(name = "UpdateNonFaculty", urlPatterns = {"/UpdateNonFaculty"})
@MultipartConfig(fileSizeThreshold = 1024*1024*10, maxFileSize = 1024*1024*50, maxRequestSize = 1024*1024*100)
public class UpdateNonFaculty extends HttpServlet {
/**
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            PreparedStatement ps = conn.prepareStatement("update nonfaculty set empid=?, dob=?, emno=?, email=?, ename=?, aadhar=?, gender=?,doj=?, qual=?, designation=?, course=?, dept=?, photo=? where empid='"+empid+"'");
